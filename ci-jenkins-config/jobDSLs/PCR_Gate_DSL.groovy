...
projectsFile.eachLine {
    project_details ->
        if (project_details.contains(jenkins_instance) && !project_details.contains("EXC-PCR") && project_details.contains("SILENT_MODE=false") && project_details.contains("PCR=unit")){
            ...
            if (project_details.contains("SKIP_SONAR=false")){
                createPipelinePreCodeUnitJob(project_name,"true",env_map)
            } else {
                createPipelinePreCodeUnitJob(project_name,"false",env_map)
            }
        }
}

def createPipelinePreCodeUnitJob(project_name,gating,env_map) {
