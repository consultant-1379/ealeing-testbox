...
projectsFile.eachLine {
    project_details ->
        if (project_details.contains(jenkins_instance) && !project_details.contains("EXC-PCR") && project_details.contains("SILENT_MODE=true") && project_details.contains("PCR=integration")){
            ...
            createPipelinePreCodeIntegrationJob(project_name,env_map)
        }
}

def createPipelinePreCodeIntegrationJob(project_name,env_map) {
    ...
    pipelineJob(pipeline_name) {
        ...
        triggers {
            ...
            definition {
                ...
                cps {
                    script(readFileFromWorkspace('ci-jenkins-config/jenkinsFiles/ciPipelineJenkinsFiles/pcr_Gate_Integration.groovy'))
                    sandbox(true)
                }
            }
        }
